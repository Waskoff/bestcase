...
def smuta(s):
    ...
    if s > 99:
        ...
        btn_next1.grid(column=0, row=2)
        btn_next2 = Button(s_ww, text='сложить полномочия', bg='red', fg='black',
                           command=lambda: [end(), s_ww.destroy()])
        btn_next2.grid(column=1, row=2)
...
def issledovanie_zemel():
    ...
    # tx.grid(column=0, row=0)
    btn_next1 = Button(l_ww, text='Присоединено\nтерриторий:\n' + str(e), bg='green', fg='white',
                       command=lambda: [l_ww.destroy()])
    btn_next1.grid(column=0, row=2)
